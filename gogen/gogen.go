...
type goStructField struct {
	...
	// the base template.
	IsYANGList  bool
	IsMandatory bool //IsMandatory stores whether the field is mandatory. TODO New
}
...
type goUnionInterface struct {
	...
	Types                map[string]goUnionType // Types is a map keyed by the camelcase type name, with values of the Go types in the union. TODO New
...
func (b *Binary) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	...
	decodedB, err := base64.StdEncoding.DecodeString(s)
	if err != nil {
		return err
	}
	*b = decodedB
...
{{ $dtr := .NameDelimiter -}}
// E{{ $dtr }}{{ .EnumerationPrefix }} is a derived int64 type which is used to represent
// the enumerated node {{ .EnumerationPrefix }}. An additional value named
// {{ .EnumerationPrefix }}{{ $dtr }}UNSET is added to the enumeration which is used as
// the nil value, indicating that the enumeration was not explicitly set by
// the program importing the generated structures.
type E{{ $dtr }}{{ .EnumerationPrefix }} int64
...
func (E{{ $dtr }}{{ .EnumerationPrefix }}) IsYANGGoEnum() {}

// ΛMap returns the value lookup map associated with  {{ .EnumerationPrefix }}.
func (E{{ $dtr }}{{ .EnumerationPrefix }}) ΛMap() map[string]map[int64]ygot.EnumDefinition { return ΛEnum; }

// String returns a logging-friendly string for E{{ $dtr }}{{ .EnumerationPrefix }}.
func (e E{{ $dtr }}{{ .EnumerationPrefix }}) String() string {
	return ygot.EnumLogString(e, int64(e), "E{{ $dtr }}{{ .EnumerationPrefix }}")
}

{{ $enumName := .EnumerationPrefix -}}
const (
	{{- range $i, $val := .Values }}
	// {{ $enumName }}{{ $dtr }}{{ $val }} corresponds to the value {{ $val }} of {{ $enumName }}
	{{ $enumName }}{{ $dtr }}{{ $val }} E{{ $dtr }}{{ $enumName }} = {{ $i }}
	{{- end }}
)
`)

	// goEnumStrDefinitionTemplate takes an input generatedGoEnumeration struct
	// and outputs the Go code that is associated with the enumerated type to be
	// generated.
	goEnumStrDefinitionTemplate = mustMakeTemplate("enumStrDefinition", `
{{ $dtr := .NameDelimiter -}}
// E{{ $dtr }}{{ .EnumerationPrefix }} is a derived string type.
// +kubebuilder:validation:Enum=
{{- range $i, $val := .Values -}}
{{- if $i -}};{{- end -}}
{{ $val }}
{{- end }}
type E{{ $dtr }}{{ .EnumerationPrefix }} string

{{ $enumName := .EnumerationPrefix -}}
const (
	{{- range $i, $val := .Values }}
	// {{ $enumName }}{{ $dtr }}{{ $val }} corresponds to the value {{ $val }} of {{ $enumName }}
	{{ $enumName }}{{ $dtr }}{{ $val }} E{{ $dtr }}{{ $enumName }} = "{{ $val }}"
	{{- end }}
)
`)

	// goNewListMemberTemplateAsList takes an input generatedGoListMethod struct and
	...
	// function.
	goNewListMemberTemplateAsList = mustMakeTemplate("newListEntryAsList", `
...
// arguments.
func (t *{{ .Receiver }}) New{{ .ListName }}() (*{{ .ListType }}, error){
	// Initialise the list within the receiver struct if it has not already been
	// created.
	if t.{{ .ListName }} == nil {
		t.{{ .ListName }} = make([]*{{ .ListType }}, 0)
	}

	item := &{{ .ListType }}{}
	t.{{ .ListName }} = append(t.{{ .ListName }}, item)

	return item, nil
}
`)

	// goNewListMemberTemplateAsMap takes an input generatedGoListMethod struct and
	// outputs a method, using the specified receiver, that creates a new instance
	// of a struct within a keyed YANG list, and populates the map key, and the
	// key fields of the list's struct according to the input arguments of the
	// function.
	goNewListMemberTemplateAsMap = mustMakeTemplate("newListEntry", `
// New{{ .ListName }} creates a new entry in the {{ .ListName }} list of the
// {{ .Receiver}} struct. The keys of the list are populated from the input
// arguments.
...
`)
	...
	// goListAppendTemplateAsList defines a template for a function that takes an
	...
	// code.
	goListAppendTemplateAsList = mustMakeTemplate("appendListAsList", `
...
func (t *{{ .Receiver }}) Append{{ .ListName }}(v *{{ .ListType }}) error {
	...
	t.{{ .ListName }} = append(t.{{ .ListName }}, v)
	return nil
}
`)
	...
	// ensure that we dereference elements that are pointers in the generated
	// code.
	goListAppendTemplateAsMap = mustMakeTemplate("appendList", `
// Append{{ .ListName }} appends the supplied {{ .ListType }} struct to the
// list {{ .ListName }} of {{ .Receiver }}. If the key value(s) specified in
// the supplied {{ .ListType }} already exist in the list, an error is
// returned.
...
var ΛEnum = map[string]map[int64]ygot.EnumDefinition{
	{{- range $enumName, $enumValues := .Enums }}
	"E{{ $dtr }}{{ $enumName }}": {
...
// {{ $intfName }}{{ $dtr }}{{ $typeName }} is used when {{ $path }}
// is to be set to a {{ $type.Name }} value.
type {{ $intfName }}{{ $dtr }}{{ $typeName }} struct {
	{{ $typeName }}	{{ $type.Name }}
}

// Is{{ $dtr }}{{ $intfName }} ensures that {{ $intfName }}{{ $dtr }}{{ $typeName }}
// implements the {{ $intfName }} interface.
func (*{{ $intfName }}{{ $dtr }}{{ $typeName }}) Is{{ $dtr }}{{ $intfName }}() {}
{{ end -}}
`)
...
{{- $path := .LeafPath }}
{{- $dtr := .NameDelimiter }}
// To{{ $dtr }}{{ .Name }} takes an input interface{} and attempts to convert it to a struct
// which implements the {{ .Name }} union. It returns an error if the interface{} supplied
// cannot be converted to a type within the union.
func (t *{{ .ParentReceiver }}) To{{ $dtr }}{{ .Name }}(i interface{}) ({{ .Name }}, error) {
	switch v := i.(type) {
	{{ range $typeName, $type := .Types -}}
	case {{ $type.Name }}:
		return &{{ $intfName }}{{ $dtr }}{{ $typeName }}{v}, nil
	{{ end -}}
	default:
...
`)
	// unionTypeNoInterfaceTemplate outputs the type that corresponds to a multi-type union
	// in the YANG schema. It does so by enhancing generated typedefs.
	unionTypeNoInterfaceTemplate = mustMakeTemplate("unionTypeNoInterface", `
{{ $intfName := .Name -}}
// {{ $intfName }} is an interface that is implemented by valid types for the union
// for the leaf {{ .LeafPath }} within the YANG schema.

// +enum
type {{ $intfName }}Discriminator string

const (
{{- range $typeName, $type := .Types -}}
	{{ $intfName }}{{ $typeName }} {{ $intfName }}Discriminator = "{{ $intfName }}{{ $typeName }}"
{{ end -}}
	{{ $intfName }}None {{ $intfName }}Discriminator = ""
)

type {{ $intfName }} struct {
	// +unionDiscriminator
	// +required
	Discriminator {{ $intfName }}Discriminator `+"`"+`json:"discriminator"`+"`"+`
{{- range $typeName, $type := .Types }}
{{- range $i, $annotation := $type.Annotations }}
	{{ $annotation }}
{{- end }}
	{{ $intfName }}{{ $typeName }}	{{ $type.Name }} `+"`"+`{{ $type.Tags }}`+"`"+`
{{- end }}
}
`)
	// unionTypeNoInterfaceTemplate outputs the type that corresponds to a multi-type union
	// in the YANG schema. It does so by enhancing generated typedefs.
	unionTypeIntOrStringTemplate = mustMakeTemplate("unionTypeIntOrString", `
{{ $intfName := .Name -}}
// {{ $intfName }} is an interface that is implemented by valid types for the union
// for the leaf {{ .LeafPath }} within the YANG schema.
//+kubebuilder:validation:XIntOrString
{{- range $typeName, $type := .Types }}
{{- range $i, $pattern := $type.Pattern }}
//+kubebuilder:validation:Pattern="{{ $pattern }}"
{{- end }}
{{- end }}
type {{ $intfName }} intstr.IntOrString
func (in *{{ $intfName }}) UnmarshalJSON(value []byte) error {
	if value[0] == '"' {
		in.Type = intstr.String
		return json.Unmarshal(value, &in.StrVal)
	}
	in.Type = intstr.Int
	return json.Unmarshal(value, &in.IntVal)
}
func (in *{{ $intfName }}) MarshalXML(e *xml.Encoder, start xml.StartElement) error {
	if in.Type == intstr.Int {
		return e.EncodeElement(in.IntVal, start)
	} else {
		return e.EncodeElement(in.StrVal, start)
	}
}
func (in *{{ $intfName }}) UnmarshalXML(d *xml.Decoder, start xml.StartElement) error {
	var s string
	err := d.DecodeElement(&s, &start)
	if err != nil {
		return err
	}
	*in = ({{ $intfName }})(intstr.Parse(s))
	return nil
}
`)
...
type {{ .Name }} interface {
	// Union type can be one of [{{ .SubtypeDocumentation }}]
	Documentation{{ $dtr }}for{{ $dtr }}{{ .Name }}()
}
{{ $intfName := .Name -}}
{{- $path := .LeafPath -}}
{{- range $typeName, $type := .Types }}
// Documentation{{ $dtr }}for{{ $dtr }}{{ $intfName }} ensures that {{ $typeName }}
// implements the {{ $intfName }} interface.
func ({{ $typeName }}) Documentation{{ $dtr }}for{{ $dtr }}{{ $intfName }}() {}
{{ end -}}
`)
...
func writeGoHeader(yangFiles, includePaths []string, cfg *CodeGenerator, rootName string, modelData []*gpb.ModelData) (string, string, error) {
	...
	s := struct {
		...
		ModelData:        modelData,
		HeaderComment:    cfg.GoOptions.HeaderComment,
	}
...
func writeGoStruct(targetStruct *ygen.ParsedDirectory, goStructElements map[string]*ygen.ParsedDirectory, generatedUnions map[string]bool, goOpts GoOpts, additionalFields []*goStructField, mapper *GoLangMapper, namespace string) (GoStructCodeSnippet, []error) {
	if targetStruct == nil {
	...
	nameDelimiter := "_"
	if goOpts.NameDelimiter != nil {
		nameDelimiter = *goOpts.NameDelimiter
	}

	// Alphabetically order fields to produce deterministic output.
	for _, fName := range orderedFieldNames(targetStruct, goOpts.GenerateXMLTag) {
		...
		if goOpts.GenerateUnionsIntOrString && field.LangType != nil && field.LangType.UnionTypes != nil {
			field.LangType.ZeroValue = fmt.Sprintf(`(%s)(intstr.FromInt(0))`, field.LangType.NativeType)
			if len(field.YANGDetails.Defaults) > 0 {
				def := fmt.Sprintf(`(%s)(intstr.Parse("%s"))`, field.LangType.NativeType, field.YANGDetails.Defaults[0])
				field.LangType.DefaultValue = &def
			}
		}
		...
		case ygen.ListNode:
			...
			fieldType, multiKeyListKey, listMethods, listErr := yangListFieldToGoType(field, fieldName, targetStruct, goStructElements, nameDelimiter, goOpts.GenerateListMemberAsList)
			if listErr != nil {
		...
		case ygen.LeafNode, ygen.LeafListNode:
			fixedPath := strings.TrimPrefix(field.YANGDetails.Path, "/"+field.YANGDetails.RootElementModule)
			entry, err := mapper.LangMapperBase.ResolveLeafrefTarget(fixedPath, nil)
			...
			if len(field.LangType.UnionTypes) > 1 && !genUnionSet[field.LangType.NativeType] {
				...
				var genTypes []string
				for t, unionType := range field.LangType.UnionTypes {
					tn := yang.CamelCase(t)
					...
					if goOpts.GenerateJSONTag {
						tagBuf.WriteString(fmt.Sprintf(` json:"%s,omitempty"`, igenutil.Convert2CamelCase(t)))
					}
					if goOpts.GenerateXMLTag {
						tagBuf.WriteString(fmt.Sprintf(` xml:"%s"`, t))
					}
					var annotations []string
					if goOpts.GenerateUnionsNoInterface {
						annotations = []string{"// +unionMember,optional", "// +optional"}
					}

					var yangTypePattern []string
					if entry != nil && entry.Type != nil && entry.Type.Type != nil {
						eT := entry.Type.Type[unionType.Index]
						if eT.Enum != nil {
							yangTypePattern = []string{strings.Join(eT.Enum.Names(), "|")}
						} else {
							patterns := make([]string, 0)
							for _, p := range eT.Pattern {
								patterns = append(patterns, xsd2kubePattern(p))
							}
							yangTypePattern = patterns
						}
					}
					intf.Types[tn] = goUnionType{
						Name:        t,
						Tags:        tagBuf.String(),
						Pattern:     yangTypePattern,
			...
			isMandatory := false
			if err == nil {
				isMandatory = entry.Mandatory.Value()
			}
			fieldDef = &goStructField{
				Name:          fieldName,
				Type:          fType,
		...
		if goOpts.GenerateJSONTag {
			...
			tagBuf.WriteString(fmt.Sprintf(" json:\"%s%s\"", igenutil.Convert2CamelCase(field.Name), omitEmpty))
		}

		// Append a xml tag.
		if goOpts.GenerateXMLTag {
			tagBuf.WriteString(fmt.Sprintf(" xml:\"%s\"", field.YANGDetails.Name))
		}
	...
	for _, method := range associatedListMethods {
		if goOpts.GenerateListMemberAsList {
			if err := goNewListMemberTemplateAsList.Execute(&methodBuf, method); err != nil {
				errs = append(errs, err)
			}
		} else {
			if err := goNewListMemberTemplateAsMap.Execute(&methodBuf, method); err != nil {
				errs = append(errs, err)
			}
		}
		...
		if goOpts.GenerateAppendMethod {
			if goOpts.GenerateListMemberAsList {
				if err := generateListAppendAsList(&methodBuf, method); err != nil {
					errs = append(errs, err)
				}
			} else {
				if err := generateListAppendAsMap(&methodBuf, method); err != nil {
					errs = append(errs, err)
				}
			}
		}
	...
	for _, intf := range genUnions {
		if goOpts.GenerateUnionsIntOrString {
			if _, ok := generatedUnions[intf.Name]; !ok {
				if err := unionTypeIntOrStringTemplate.Execute(&interfaceBuf, intf); err != nil {
					errs = append(errs, err)
				}
				generatedUnions[intf.Name] = true
			}
		} else if goOpts.GenerateUnionsNoInterface {
			if _, ok := generatedUnions[intf.Name]; !ok {
				if err := unionTypeNoInterfaceTemplate.Execute(&interfaceBuf, intf); err != nil {
					errs = append(errs, err)
				}
				generatedUnions[intf.Name] = true
			}
		} else if goOpts.GenerateSimpleUnions {
			if _, ok := generatedUnions[intf.Name]; !ok {
	...
	pathItems := strings.Split(targetStruct.Path, "/")
	...
	return GoStructCodeSnippet{
		...
		ConfigFalse: targetStruct.ConfigFalse,
		Root:        strings.Count(targetStruct.Path, "/") == 2,
		YangName:    yangName,
		Namespace:   namespace,
	}, errs
}

func xsd2kubePattern(pattern string) string {
...
// generateListAppendAsList generates a function which appends a (key, value) to a
...
func generateListAppendAsList(buf *bytes.Buffer, method *generatedGoListMethod) error {
	return goListAppendTemplateAsList.Execute(buf, method)
}

// generateListAppendAsMap generates a function which appends a (key, value) to a
// Go map (YANG list) within the generated code. The argument of the generated
// function is the map's member type - from which the key values are extracted.
// The generated function returns an error if the key already exists in the list.
//
// The generated function is written to the supplied buffer - using the supplied
// method argument to determine the list's characteristics in the template.
func generateListAppendAsMap(buf *bytes.Buffer, method *generatedGoListMethod) error {
	return goListAppendTemplateAsMap.Execute(buf, method)
}
...
func yangListFieldToGoType(listField *ygen.NodeDetails, listFieldName string, parent *ygen.ParsedDirectory, goStructElements map[string]*ygen.ParsedDirectory, delimiter string, asList bool) (string, *generatedGoMultiKeyListStruct, *generatedGoListMethod, error) {
	...
	case len(listElem.ListKeys) == 1:
		...
		if asList {
			listType = fmt.Sprintf("[]*%s", listElem.Name)
		} else {
	...
	default:
		...
		// definition.
		listKeyStructName = fmt.Sprintf("%s%sKey", listElem.Name, delimiter)
		names := make(map[string]bool, len(goStructElements))
		for _, d := range goStructElements {
			names[d.Name] = true
		}
		if names[listKeyStructName] {
			listKeyStructName = fmt.Sprintf("%s%s%s%sYANGListKey", parent.Name, delimiter, listFieldName, delimiter)
			if names[listKeyStructName] {
...
func writeGoEnum(inputEnum *goEnumeratedType, nameDelimiter string) (string, error) {
...
// writeGoEnumStr takes an input goEnumeratedType, and generates the code corresponding
// to it. If errors are encountered whilst mapping the enumeration to
// code, they are returned. The enumDefinition template is used to convert a
// constructed generatedGoEnumeration struct to code within the function.
func writeGoEnumStr(inputEnum *goEnumeratedType, nameDelimiter string) (string, error) {
	var buf strings.Builder
	if err := goEnumStrDefinitionTemplate.Execute(&buf, generatedGoEnumeration{
		EnumerationPrefix: inputEnum.Name,
		Values:            inputEnum.CodeValues,
		NameDelimiter:     nameDelimiter,
	}); err != nil {
...
// programmatically.
func writeGoEnumMap(enums map[string]map[int64]ygot.EnumDefinition, nameDelimiter string) (string, error) {
	if len(enums) == 0 {
		return "", nil
	}
	...
	var buf bytes.Buffer
	in := struct {
		Enums         map[string]map[int64]ygot.EnumDefinition
		NameDelimiter string
	}{
		Enums:         enums,
		NameDelimiter: nameDelimiter,
	}
	if err := goEnumMapTemplate.Execute(&buf, in); err != nil {
		return "", err
	}
	return buf.String(), nil
}
...
func goLeafDefaults(e *yang.Entry, t *ygen.MappedType, goEnumPrefix, nameDelimiter string) []string {
	...
	for i, defVal := range defaultValues {
		if t.IsEnumeratedValue {
			defaultValues[i] = enumDefaultValue(t.NativeType, defVal, goEnumPrefix, nameDelimiter)
		} else {
...
func orderedFieldNames(d *ygen.ParsedDirectory, useKeyOrder bool) []string {
	if !useKeyOrder {
		return d.OrderedFieldNames()
	}
	if d == nil {
		return nil
	}
	...
	sort.Slice(fieldNames, func(i, j int) bool {
		_, okI := d.ListKeys[fieldNames[i]]
		_, okJ := d.ListKeys[fieldNames[j]]
		if (okI && okJ) || (!okI && !okJ) {
